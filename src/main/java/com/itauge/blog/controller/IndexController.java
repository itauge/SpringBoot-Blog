...
public class IndexController {
    ...
    public String index(@PageableDefault(size = 8,sort = {"updateTime"},direction = Sort.Direction.DESC)Pageable pageable, Model model){
        ...
        model.addAttribute("recommendBlog",blogService.listRecommedBlogTop(5));
        return "index";
    }

    @PostMapping("/search")
    public String search(@RequestParam String query,
                         @PageableDefault(size = 4,direction = Sort.Direction.DESC,sort = {"createTime"}) Pageable pageable,
                         Model model){
        //"%"+query+"%"模糊查詢
    ...
    public String blog(@PathVariable("id") Long id, Model model){
        ...
        model.addAttribute("comments",commentService.listCommentParentIdIsNull(id));
        return "blog";
    }
}
